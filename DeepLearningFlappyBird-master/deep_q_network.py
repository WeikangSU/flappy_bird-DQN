...
# 训练神经网络
# s_t是每轮的第一个状态，s_t1是当前状态，每一step结束即替换状态，准备下一轮step
def trainNetwork(s, readout, h_fc1, sess):
    ...
    x_t, r_0, terminal, score = game_state.frame_step(do_nothing)  # do_nothing动作输入到step(),返回观察值，reward，和是否结束
    ...
    while True:
        ...
        if t > OBSERVE:
            ...
            y_batch = []
            # 把上面获得的 s_j1_batch（下一个state）feed到神经网络，获得下一个动作的q值
            readout_j1_batch = readout.eval(feed_dict = {s : s_j1_batch})
            for i in range(0, len(minibatch)):      # 对于minibatch里面的每一个sample：
                terminal = minibatch[i][4]      # minibatch的[4]位置对应terminal
                # if terminal, only equals reward
                if terminal:        # terminal: 只返回reward作为y值，因为没有下一个action和state
                    y_batch.append(r_batch[i])
                else:
                    y_batch.append(r_batch[i] + GAMMA * np.max(readout_j1_batch[i]))    #non-terminal:根据下一步的state计算出当前状态的q值
